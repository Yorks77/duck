...
class AFKModeMod(loader.Module):
    ...
    async def allowcmd(self, message):
        ...
        except: return await utils.answer(message, self.strings["who_to_allow"])
        ...
        try:
            if message.is_private:
                ...
                if reply:
                    ...
                    user = await message.client.get_entity(int(args) if args.isnumeric() else args)
        except: return await utils.answer(message, self.strings["who_to_deny"])
    ...
    async def allowedcmd(self, message):
        ...
        for _ in allowed:
            ...
            users += f"{number}. <a href=\"tg://user?id={int(_)}\">{user.first_name}</a> | [<code>{_}</code>]\n"
